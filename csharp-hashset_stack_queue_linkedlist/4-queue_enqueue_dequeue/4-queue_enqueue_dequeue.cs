...
class MyQueue
{
    ...
    {
        int Nitems = aQueue.Count; // un integer ou on stock le count de la Queue
        Console.WriteLine("Number of items: {0}", Nitems); // on le writreline
        if (Nitems <= 0)
            Console.WriteLine("Queue is empty"); // c'est vide on le dit
        else  
            Console.WriteLine("First item: {0}", aQueue.Peek());//  sinon on print le premier item
        ...
        return (aQueue); // et on retourne la Queue
    }
}